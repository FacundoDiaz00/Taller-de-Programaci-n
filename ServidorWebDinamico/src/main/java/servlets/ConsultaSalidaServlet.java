		...
		try {
			...
			if (sesion != null && sesion.getAttribute("usuarioLogeado") != null) {
				...
				if (marcarActividadComoFav) {
					wbUsuarios.agregarOEliminarActividadDeFavoritos(user.getNickname(),
							infoSalidaTuristica.getActividad());
				}

			}

			if (user instanceof DtTurista) {
				esFavorito = wbUsuarios.perteneceAFavoritosDeTurista(user.getNickname(),
						infoSalidaTuristica.getActividad());
			}
			...
			String msg = "El usuario logeado no está registrado en el sistema";

			if (e.getFaultInfo().getClaseObjetoFaltante().equals("ActividadTuristica"))
				msg = "Se intentó hacer favorito a una actividad finalizada.";

			req.setAttribute("motivoDeError", msg);
			req.getRequestDispatcher("/WEB-INF/jsp/errores/400.jsp").forward(req, resp);
			return;
		}
